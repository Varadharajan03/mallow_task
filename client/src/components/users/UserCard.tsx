            ...
            <div className="relative h-full w-full transition-all duration-700 ease-in-out [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] hover:scale-105">
                <div className="absolute inset-0 [backface-visibility:hidden]">
                    <div className="h-full bg-gradient-to-br from-amber-50 to-orange-100 rounded-2xl shadow-lg hover:shadow-2xl border border-amber-200 p-6 flex flex-col items-center justify-center text-center transition-all duration-500 transform hover:-translate-y-1">
                        ...
                        <h3 className="text-xl font-bold text-gray-800 mb-2 transform group-hover:translate-y-1 transition-transform duration-500 truncate w-full px-2">
                            {`${user.first_name} ${user.last_name}`}
                        </h3>
                        
                        <div className="text-center space-y-1 transform group-hover:translate-y-1 transition-transform duration-500 delay-75 w-full">
                            <p className="text-sm text-gray-600 font-medium truncate px-2">{user.email}</p>
                            {user.job_title && (
                                <p className="text-xs text-amber-700 bg-amber-100 px-3 py-1 rounded-full truncate mx-auto max-w-[180px]">
                                    {user.job_title}
                                </p>
                            )}
                        </div>
                        ...
                        <div className="mt-4 text-xs text-gray-500 opacity-70 group-hover:opacity-100 transition-opacity duration-300">
                            <div className="flex items-center space-x-1">
                                <svg className="h-3 w-3 animate-bounce flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)]">
                    <div className="h-full bg-white rounded-2xl shadow-2xl border border-amber-200 p-6 flex flex-col overflow-hidden">
                        <div className="text-center mb-4 pb-4 border-b border-gray-100 flex-shrink-0">
                        ...
                        <div className="flex-1 space-y-3 text-sm overflow-y-auto min-h-0">
                            {user.job_title && (
                                <div className="flex items-center space-x-2">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <span className="font-medium text-gray-800 truncate block">{user.job_title}</span>
                                        {user.company && <span className="text-gray-600 truncate block">@ {user.company}</span>}
                                    </div>
                                </div>
                            )}
                            ...
                            {user.website && (
                                <div className="flex items-center space-x-2">
                                    ...
                                    <a 
                                        ...
                                        target="_blank" 
                                        rel="noreferrer"
                                        className="text-amber-600 hover:text-amber-800 transition-colors duration-200 truncate hover:underline min-w-0"
                                    >
                        ...
                        <div className="flex space-x-2 mt-4 pt-4 border-t border-gray-100 flex-shrink-0">
                            <button 
                                onClick={() => onEdit(user)} 
